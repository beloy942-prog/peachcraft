...
- **Admin** (guarded by `src/lib/adminMiddleware.ts` + server `verifyAdmin()`):
  ...
  - `admin/customers/index.tsx`, `admin/newsletter.tsx` (**newsletter subscribers list — search + 25/page pagination**), `admin/messages.tsx` (**contact messages — search, new/read badges, mark-as-read mutation w/ exact-key invalidation**), `admin/website-settings.tsx` (store details + **GCash fields**).
...
- `order_items` — `order_id → orders` (**NO cascade at table-creation time**; migration 013 re-adds the FK WITH `on delete cascade` — memory previously misstated this as cascading from the start), `product_id → products`, `qty`, `price_at_purchase`.
- `gcash_payments` — `id, order_id → orders (CASCADE since migration 004), gcash_reference_number (UNIQUE), screenshot_url, customer_email, status (pending|verified|rejected), submitted_at, verified_at`.
...
- `011_add_newsletter_subscribers.sql` (**NEW 2026-08-25 — NOT YET APPLIED to Supabase**): `newsletter_subscribers` (`id uuid pk default gen_random_uuid()`, `email text NOT NULL UNIQUE`, `created_at timestamptz default now()`). RLS enabled with **INSERT-only policy for anon+authenticated** (no client SELECT/UPDATE/DELETE); admin reads via service-role. Plus DB CHECK constraint on email format (guards direct anon-key PostgREST inserts that bypass app zod). Idempotent. Apply via dashboard before deploying the newsletter code.
- `012_add_contact_messages.sql` (**NEW 2026-08-25 — NOT YET APPLIED to Supabase**): `contact_messages` (`id uuid pk`, `name text`, `email text`, `message text`, `status text default 'new'`, `created_at timestamptz`). RLS INSERT-only for anon+authenticated; DB CHECKs: email format, name ≤100, message 1–2000, status in ('new','read'); index on created_at desc. Admin reads/status updates via service-role. No IP rate limit yet (flagged as follow-up). Apply via dashboard before deploying the contact-form code.
- `013_order_items_cascade_delete.sql` (**NEW 2026-08-25 — NOT YET APPLIED to Supabase**): drops + re-adds `order_items_order_id_fkey` WITH `on delete cascade`. REQUIRED before deploying admin order hard-delete (`deleteOrder`), which relies on a single-statement `DELETE FROM orders` cascading to order_items (+ gcash_payments via 004) atomically — supabase-js has no multi-statement transactions.
...
| `getOrdersList` | 1262 | GET | admin; user_email = profile email, **falls back to `shipping_address.email` for guests** ("Unknown" only if neither exists) |
| `getOrderDetails` | 1300 | GET | admin; **FIXED: `let user=null; if(order.user_id){…maybeSingle()}` — no throw on null/missing profile**; **customer.name/email fall back to `shipping_address` for guest orders** |
| `updateOrderStatus` | 1390 | POST | admin; **restores stock when status → cancelled** |
| `deleteOrder` | 1429 | POST | admin hard-delete; restores stock ONLY if status ≠ cancelled (cancelled already restored by cancel flow — double-restore would inflate); single DELETE on orders cascades to order_items (migration 013) + gcash_payments (004); destroys payment/audit records — see §9 #27 |
...
- `newsletter.functions.ts` — `subscribeNewsletter` (POST): zod email, lowercases + inserts into `newsletter_subscribers`; catches PG `23505` → friendly `{alreadySubscribed:true}` (no throw). `getNewsletterSubscribers` (GET): admin-guarded via `verifyAdmin()`, newest-first email+created_at list.
- `contact.functions.ts` — `submitContactMessage` (POST): zod name ≤100 / email / message 1–2000, trims, lowercases email, inserts `contact_messages(new)`. `getContactMessages` (GET): admin-guarded, newest-first. `updateContactMessageStatus` (POST): admin-guarded, idempotent status update (`new`|`read`).
...
- Step 3: GCash panel (QR/number/name/amount from `gcashConfig`; **Order ID = prominent chip directly below Amount to Pay** — `bg-blush` + `border-wine/40` chip, `text-wine` bold mono `text-base`, Copy button; when `displayOrderId` is absent the same position/label shows a dashed-border placeholder "Generated when you submit your proof below" so layout doesn't shift) + Important Instructions (manual-numbered `<p>`s, NOT an `<ol>` — renumber when editing; 3 steps: send amount / screenshot confirmation / fill ref number + upload proof — the GCash-note "enter ORDER ID without dashes" instruction was REMOVED 2026-08-25) + proof form + Back / **Submit Payment Proof = the explicit confirm action** (`handleSubmitProof`: proof-field checks → dup-ref check → `validateCheckout()` → `placeOrder()` (creates order, reuses existing `orderId` on retry so partial failure never double-orders) → upload screenshot → `submitGCashProof`). COD step 3 = review summary + "Confirm Order" button (`handleConfirmCodOrder`). Success → `setStep(4)` success screen. Stored/displayed order ID keeps its dashes; only instruction #2 tells customers to strip dashes in the GCash note (no auto-formatting).
...
1. **Admin dashboard crash (OPEN)** — `admin/index.tsx:138` `data!.revenueByMonth` throws when `data` is `undefined` with `isLoading=false, error=null`. Same `data!` at ~143,162,221,230,239,266,270,307. Root cause unconfirmed; underlying DB queries verified working. *§9 #1 (admin dashboard crash) — confirmed still present, unrelated to custom-category change (2026-08-26).*
...
10. **M3 (FIXED 2026-08-25)** — Contact form wired: `contact.tsx` → `submitContactMessage` server fn → `contact_messages` (migration **012 — PENDING APPLY**); controlled fields, sonner toasts, cleared on success, disabled + "Sending..." pending state. Admin inbox at `/admin/messages` (sidebar "Messages" next to "Newsletter"). No IP rate limit yet — flagged follow-up.
11. **M4 (FIXED 2026-08-25)** — Newsletter signup now wired: footer form → `subscribeNewsletter` server fn → `newsletter_subscribers` (migration **011 — PENDING APPLY**); success/dup/invalid toasts via sonner; input cleared + "You're in! 🍑" on submit. Admin list at `/admin/newsletter` (sidebar entry added). Accepted-payments block + `paymentMethods` const removed from `SiteFooter.tsx`.
...
27. **Order hard-delete audit risk (NEW 2026-08-25, accepted-by-design — review)** — `deleteOrder` permanently destroys the order, its line items, and linked `gcash_payments` rows (incl. proof URLs + reference numbers). Consequences to keep in mind: (a) financial/audit trail for that order is gone; (b) the UNIQUE `gcash_reference_number` is freed and could be re-submitted on a new order (fraud-replay vector if deleting a fraudulent order); (c) stock is restored only when deleted order ≠ cancelled (prevents double-restore inflation). Delete is currently allowed for ALL statuses incl. shipped/delivered/paid — flagged; consider restricting or soft-deleting later.
28. **Authenticated-order email mismatch (FIXED 2026-08-26)** — Server-side email match for authenticated orders was missing: the guest path (`submitGCashProof`, line ~748-750) already rejected a GCash proof if the submitted email didn't match `shipping_address.email`, but the authenticated path only checked `user_id` ownership — it never compared account email to shipping email. A logged-in customer could submit a payment proof with a shipping email different from their account email and it would be accepted regardless. Fixed by applying the same email-match check to both guest and authenticated paths (`supabase.functions.ts:766-771`).
29. **Custom category deletion (ADDED 2026-08-26)** — Category & tags section (`ProductForm.tsx`) now supports deleting custom categories via trash icon → AlertDialog confirmation (category name + Cancel/Delete) → server delete. `custom_categories` table added (migration 015); `addCategory` now also calls `createCustomCategory` to persist (non-blocking: create/delete are best-effort against the table — if persist fails, category still applies to the current product form state, so a save could succeed with a category not yet in the table). Built-in categories (Necklaces, Airdry Clay Crafts, Fake Cakes) are not deletable. `["custom-categories"]` React Query key invalidated after create/delete.

Open question carried forward: in-use handling — confirm/flag whether deleting a category currently in use by existing products is blocked, reassigns them, or leaves a stale string. Not resolved in this session per report above — needs explicit decision before considered fully closed.
...
- **When adding validation to one path (guest vs. authenticated), always check the sibling path** — guest and authenticated flows share intent but are implemented separately in this codebase and tend to drift (see #28).
...
- ⚠️ OPEN: **migrations 010 (product detail cols) + 011 (newsletter_subscribers) + 012 (contact_messages) + 013 (order_items cascade) not applied to Supabase — apply ALL via dashboard before deploying their code**; Vercel 500 (#2); admin dashboard crash (#1); audit fixes H1/M1/M5/M6/L1-L6/L8 pending user confirmation (M3+M4 fixed 2026-08-25); optional: contact/newsletter IP rate limit (`*_attempts` pattern), new-message nav badge (getAdminNotifications extension), DB CHECK on orders.status (#25 follow-up), delete-status restriction review (#27).
